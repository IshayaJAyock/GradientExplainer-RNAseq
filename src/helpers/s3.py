...
def get_files_in_bucket(bucket_name, folder, suffix=None, file_contains=None):
    ...
    for page in s3_paginator.paginate(Bucket=bucket_name, Prefix=folder):
        for content in page.get('Contents', ()):
            ...
            if (suffix is None or file_key.lower().endswith(suffix)) and (file_contains is None or file_contains in file_key):
...
def download_file(bucket_name, file_key, local_file_name=None, suffix=None):
    ...
    if file_name is None:
        file_name = tempfile.mkstemp(suffix=suffix)[1]
    else:
        make_dirs_for_file(file_name)
    s3_client = boto3.client('s3')
    # download
    try:
        s3_client.download_file(Bucket=bucket_name, Key=file_key, Filename=file_name)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == "404":
            raise FileNotFoundError
    return file_name

def download_file_if_needed(bucket_name, file_key, local_file_name, overwrite=False, verbose=False):
    """
...
def download_folder(bucket_name, folder, output_dir, overwrite=False, verbose=False):
    ...
    if verbose: print("Found {} files in s3 folder {}: {}".format(len(files), folder, ", ".join(files)))
    # Download each file
    for file in files:
        local_file = os.path.join(output_dir, file[len(folder) + 1:]) # Remove base folder
        # Check if its a folder, if it is we don't want to download it - just make
        if file[-1] == "/":
            os.makedirs(local_file, exist_ok=True)
        else:
            download_file_if_needed(local_file, file, bucket_name, overwrite=overwrite, verbose=verbose)

def download_folder_if_needed(local_dir, s3_dir=None, s3_bucket=None, overwrite=False, verbose=False):
    """
    ...
    if s3_dir is not None and s3_bucket is None:
        raise ValueError("If downloading a remote s3 folder, s3_bucket must be provided")
    # Check if downloading file was requested (None indicates no remote file)
    if s3_dir is not None:
        download_folder(s3_dir, local_dir, s3_bucket, overwrite=overwrite, verbose=verbose)
...
def save_pandas_df(preds, local_csv_path, s3_csv_file_key, bucket_name):
    ...
    """
    # Check args
    if local_csv is None and s3_csv is None:
        print("Both results_csv and results_s3_csv are none, at least one must be provided.")
    # Save
    if local_csv is not None:
        preds.to_csv(local_csv, index=False)
    if s3_csv is not None:
        str_output = preds.to_csv(None, index=False)
        s3 = boto3.client('s3')
        s3.put_object(Bucket=s3_bucket, Body=str_output,
        Key=s3_csv)
...
def upload_folder_to_s3_folder(local_folder, bucket_name, folder_prefix):
    ...
    s3_prefix = s3_prefix + "/" if len(s3_prefix) > 0 else ""
    ...
    for path, subdirs, files in os.walk(local_folder):
        ...
        for file in files:
            ...
            bucket_file = s3_prefix + directory_name + file
            print("Uploading {} to {}".format(local_file, bucket_file))
            s3.upload_file(local_file, s3_bucket, bucket_file)
